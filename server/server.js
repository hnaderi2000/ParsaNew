      ...
      const result = await poolConnection.request()
        ...
        .query(`
          SELECT 
            r.name,
            u.nationalCode,
             u.userName,  -- این خط را اضافه کنید
            u.FacultyID, 
            u.DepartmentID 
          FROM user_roles ur
...
app.put("/factors/:factorId/confirm-deputy", authenticate, async (req, res) => {
  try {
    ...
    await pool.request()
      .input('factorId', sql.Int, factorId)
      .input('status', sql.Int, status)
      .input('deputyUserName', sql.NVarChar(50), deputyUserName)
      .query(`
        UPDATE factors
        SET IsConfirmedByDeputy = @status,
            DeputyNationalCode = @deputyUserName
        WHERE FactorID = @factorId
      `);
...
app.put("/factors/:factorId/confirm-research-director", authenticate, async (req, res) => {
  try {
    ...
    await pool.request()
      .input('factorId', sql.Int, factorId)
      .input('status', sql.Int, status)
      .input('researchDirectorUserName', sql.NVarChar(50), researchDirectorUserName)
      .query(`
        UPDATE factors
        SET IsConfirmedByResearchDirector = @status,
            ResearchDirectorNationalCode = @researchDirectorUserName
        WHERE FactorID = @factorId
      `);
...
app.put("/factors/:factorId/confirm-university-director", authenticate, async (req, res) => {
  try {
    ...
    await pool.request()
      .input('factorId', sql.Int, factorId)
      .input('status', sql.Int, status)
      .input('universityDeputyUserName', sql.NVarChar(50), universityDeputyUserName)
      .query(`
        UPDATE factors
        SET IsConfirmedByUniversityDeputy = @status,
            UniversityDeputyNationalCode = @universityDeputyUserName
        WHERE FactorID = @factorId
      `);
