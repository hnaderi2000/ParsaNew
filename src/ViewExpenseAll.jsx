...
// // در قسمت importها، آیکون ویرایش را اضافه کنید
...
// // تابع برای تغییر وضعیت انتخاب یک فاکتور
// const handleSelectFactor = (factorId, factor) => {
//   // بررسی شرط: آیا همه تاییدیه‌ها تایید شده‌اند
...
//   // اگر همه تایید نشده باشند، نمی‌توان انتخاب کرد
//   if (!isAllConfirmed) {
//     addNotification({
//       type: "warning",
...
// // تابع برای انتخاب/لغو انتخاب همه فاکتورهای یک thesis
...
// // تابع بررسی اینکه آیا همه فاکتورهای یک thesis قابل انتخاب هستند
...
// // تابع بررسی اینکه آیا همه فاکتورهای یک thesis انتخاب شده‌اند
...
//         if (
//           departmentFilter &&
//           thesis.DepartmentID !== parseInt(departmentFilter)
//         ) {
//           matches = false;
//         }
...
//         //   const statusValue =
//         //     statusFilter == "null" ? null : parseInt(statusFilter);
//         //   filteredFactors = factors.filter(
//         //     (f) => f.IsConfirmedByExpert == statusValue
//         //   );
...
//   // تعیین فیلد وضعیت بر اساس نقش کاربر
//   if (isExpert ) {
...
//   } else {
//     // برای سایر نقش‌ها (مدیر سیستم، عضو هیات علمی و ...) از فیلد پیش‌فرض استفاده می‌شود
//     statusFieldToCheck = "IsConfirmedByExpert";
//   }
...
// // تابع تولید گزارش درخواست هزینه پارسا
// const generateReport = async () => {
//   // یافتن فاکتورهای انتخاب شده
...
//   // اطلاعات دانشجو و استاد راهنما از اولین فاکتور
...
//   // دریافت FacultyID از اولین فاکتور
...
//   // دریافت اطلاعات کاربران با نقش‌های خاص
//   let usersData = {};
//   try {
//     const token = localStorage.getItem("token");
    
//     // درخواست با ارسال facultyId برای معاون پژوهشی دانشکده
...
//         facultyId: facultyId  // ارسال facultyId برای فیلتر کردن معاون پژوهشی دانشکده
//       },
//       headers: { Authorization: `Bearer ${token}` }
//     });
...
//   // تابع تبدیل مسیر امضا به URL کامل
...
//   // ساخت HTML گزارش
//   // ساخت HTML گزارش با استایل فشرده برای یک صفحه
//   const reportHtml = `
//     <!DOCTYPE html>
//     <html dir="rtl" lang="fa">
//     <head>
//       <meta charset="UTF-8">
//       <title>فرم درخواست هزینه پارسا</title>
//       <style>
//         * {
//           margin: 0;
//           padding: 0;
...
//          /* کادر شماره و تاریخ در گوشه سمت چپ بالا */
//         .info-box {
//           position: absolute;
//           top: 0;
//           left: 0;
...
//           <div class="form-title">فرم درخواست هزینه پارسا</div>
//           <div class="faculty-info" style="margin-top: 10px; display: inline-block;">
//             دانشکده: ${facultyName}
//           </div>
//         </div>
...
//                     ${factor.Description || ''}
//                     ${factor.FactorNumber ? `<br/><small style="color:#666;">شماره فاکتور: ${factor.FactorNumber}</small>` : ''}
//                    </td>
...
//           <div class="signature-card">
//             <div class="signature-title">تأیید معاون پژوهشی دانشکده ${facultyName}</div>
//             <p class="approval-text">
//               هزینه‌کرد فوق مطابق آئین‌نامه گرنت جامع می‌باشد و مورد تأیید است.
//             </p>
//             <div class="signature-content">
//               <div class="signature-name">
//                 نام و نام خانوادگی: 
...
//             <p class="approval-text">
//               مدیر محترم امور مالی دانشگاه پرداخت مبلغ فوق از گرنت پارسای ایشان مورد تأیید است.
//             </p>
//             <div class="signature-content">
//               <div class="signature-name">
//                 نام و نام خانوادگی: 
...
//                     <th>تاریخ</th>
//                     <th>مبلغ (ریال)</th>
//                     <th>استاد راهنما</th>  {/* ستون جدید */}
//                     <th>توضیحات</th>
                    
//                     {/* ستون وضعیت تایید کارشناس مالی */}
//                     <th>وضعیت تایید کارشناس مالی</th>
                    
//                     {/* ستون تایید کارشناس مالی */}
//                     {/* {!isSystemManager && !isDeputy && !isResearchDirector && !isUniversityDeputy && !isResearchExpert && !isProfessor && (
...
//                 </thead>
//                 <tbody>
//   {/* ردیف انتخاب همه برای این thesis */}
...
//       </label>
//        {/* دکمه درخواست هزینه پارسا - فقط برای کارشناس مالی */}
...
//                       {factors.map((factor) => {
//     // بررسی شرط تایید برای این فاکتور
//     const isAllConfirmed = 
//       factor.IsConfirmedByExpert == 1 &&
//       factor.IsConfirmedByDeputy == 1 &&
//       factor.IsConfirmedByResearchDirector == 1 &&
//       factor.IsConfirmedByUniversityDeputy ==1;
//     // پیدا کردن نام استاد راهنما
//     let professorName = '-';
...
//                       <td>
//   {factor.ProfessorName ? factor.ProfessorName : 
//    (factor.professors && factor.professors.length > 0 ? 
//      factor.professors.find(p => p.nationalCode === factor.ProfessorNationalCode)?.ProfessorName || 
//      factor.ProfessorNationalCode : 
//      '-')}
// </td>

//                       <td>{factor.Description || "-"}</td>
                      
//                       {/* وضعیت تایید کارشناس مالی */}
//                       <td>
...
//         }
//         disabled={
//           factor.IsConfirmedByDeputy == 1  // اگر معاون دانشکده تایید کرده، کارشناس مالی نتونه رد کنه
//         }
//       />
...
//           factor.IsConfirmedByExpert != 1 ||
//           factor.IsConfirmedByResearchDirector == 1  // اگر مدیر امور پژوهشی تایید کرده، معاون نتونه رد کنه
//         }
//       />
//       <span className={Styles.toggleSlider}></span>
//     </label>
...
//           factor.IsConfirmedByExpert != 1 ||
//           factor.IsConfirmedByDeputy != 1 ||
//           factor.IsConfirmedByUniversityDeputy == 1  // اگر معاون دانشگاه تایید کرده، مدیر امور پژوهشی نتونه رد کنه
//         }
//       />
//       <span className={Styles.toggleSlider}></span>
//     </label>
...
//                       {/* وضعیت تایید معاون پژوهشی دانشگاه */}
//                       <td>
...
//                         </td>
//                       )} */}
//                       {/* ستون حذف - فقط برای عضو هیات علمی */}
// {isProfessor && (
//   <td>
//     <button
//       onClick={() => openDeleteModal(factor.FactorID, factor)}
...
//       title={
//         factor.IsConfirmedByExpert == 1 
//           ? "فاکتور تایید شده توسط کارشناس مالی قابل حذف نیست" 
//           : "حذف فاکتور"
//       }
//     >
//       <FaTrash />
